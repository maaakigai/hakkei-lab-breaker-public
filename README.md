...
[![最終フェーズキュー更新前の画面遷移参考デモ](docs/images/charge-screen.png)](docs/media/keyboard-demo.mp4)

▶ [約30秒の画面遷移参考デモを再生する](docs/media/keyboard-demo.mp4)

この動画は実展示の録画ではなく、最終フェーズキュー更新前の公開版で、ローカル登録画面からキーボード予備入力によるチャージ、パンチ、破壊映像、リザルトまでの導線を記録した参考映像です。現在版では`GO!!`を基準にした日本語フェーズキューへ更新済みです。現在画面の収録には、通信しないダミーQRを表示する`release_demo_qr.bat`を使用できます。ゲームの画面遷移はmocopi入力時と共通で、動画に音声トラックはありません。
...
- Linux / systemdへのデプロイ、HTTPS、ドメイン、リバースプロキシ、会場ネットワーク、サーバーログ、バックアップ、監視、復旧手順を整備。
...
- QR参加と結果送信にtokenは使用せず、セッションIDとサーバー側の状態遷移で進行を対応付け。
...
QR、スマートフォン操作、ゲーム側の状態取得・結果送信に参加token、制御token、ゲームtokenは使用していません。管理用HTTP APIは公開せず、運用者がSSHでサーバーへ入って`manage.py`を直接実行します。イベントログもサーバー内にだけ保存し、公開APIやこのリポジトリからは取得できません。リポジトリには運用中のユーザー情報、個別スコア、セッション履歴、SSH資格情報を収録していません。
...
同梱の提出版サーバー実装には管理用HTTP APIがありません。管理確認や削除が必要な場合は、運用者がSSH接続後にサーバー内の`manage.py`を直接実行します。公開QR経路と管理経路のどちらにもtokenは使用しません。
...
2026年7月25日の提出前ローカル確認では、Node.js自動テスト300件、Pythonサーバーテスト16件、型検査、静的解析、クリーンビルド、公開素材検査、`npm audit --audit-level=high`を通過しました。デモQRモードのローカルElectronアプリを実起動し、名前登録、キーボードモードへの切替、Charge、破壊映像、Result、タイトル復帰まで完走しています。同日、運用先へ提出版サーバーを反映し、公開HTTPS / WSSでQR登録、READY、入力準備、プレイ開始、Result通知、ランキング登録、Result終了までをCLIから確認しました。mocopiとスマートフォンを組み合わせた実機完走は手動確認項目です。展示用音声は公開版に含まないため、音量バランスは公開版の確認対象外です。
...
- `release.bat`は既定でHTTPS/WSS接続、QR登録、状態同期、ランキング更新を有効にします。同梱の提出版サーバーは旧展示データと別の新規`HAKKEI_DATA_DIR`で合成初期データ10件から開始し、その後の新規登録・結果を永続保存します。QR・ゲーム経路にtokenはなく、管理用HTTP APIもありません。管理はSSH接続後に同じ`HAKKEI_DATA_DIR`を指定した`manage.py`で行い、イベントログもサーバー内にだけ保存します。2026年7月25日に運用先へ同実装を反映し、公開HTTPS / WSSの通信契約をCLIで確認済みです。スマートフォンとmocopiを使う実機完走は`release.bat`で手動確認します。
