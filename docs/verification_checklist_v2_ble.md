...
- [ ] Rank SでCriticalが成立しなかった場合は、研究室破壊動画1本だけを再生する。
- [ ] Criticalが成立した場合、または動作確認用forced Criticalを有効にした場合は、通常映像に続いて架空の大型電波塔映像を1本再生し、Resultへ650億円の表示用ボーナスを加える。ランキングはボーナス加算前のbase scoreで比較する。
